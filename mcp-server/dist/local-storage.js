...
export class LocalTaskStorage {
    ...
    validateResearchExists(task, newStatus) {
        if (newStatus === 'in_progress') {
            const hasResearch = task.research && task.research.length > 0;
            if (!hasResearch) {
    ...
    validateTimingRules(task, newStatus) {
        ...
        if (newStatus === 'done' && task.status === 'in_progress') {
            if (timeElapsed < this.MIN_WORK_TIME_MS) {
                throw new Error(`Task completed too quickly (${Math.round(timeElapsed / 1000 / 60)} minutes).\n` +
