...
import { GroupedTasks, TaskLoadingState } from '../tasks/types';
import { Task } from '../api/types';
import { getTaskService, getAuthService } from '../extension';
 ...
 */
export class TaskPanelProvider {
     ...
     */
    public showForcedApiKeyModal(): void {
        if (!this.panel) {
     ...
     */
    private async initializeTasksAsync(): Promise<void> {
        try {
     ...
     */
    public updateTasks(tasks: GroupedTasks): void {
        if (this.panel) {
            this.panel.webview.postMessage({
                command: 'tasksLoaded',
                data: tasks
    ...
    private checkForUnansweredQuestions(groupedTasks: GroupedTasks) {
        try {
            ...
            for (const task of allTasks) {
                if (task.qaHistory && task.qaHistory.length > 0) {
                    ...
                    const unansweredQA = task.qaHistory.find((qa: any) => !qa.answer);
    ...
    private calculateQueueInfo(qaHistory: any[]): { current: number, total: number } {
        const totalQuestions = qaHistory.length;
        const answeredCount = qaHistory.filter((qa: any) => qa.answer).length;
        return {
     ...
     */
    public updateLoadingState(state: TaskLoadingState): void {
        if (this.panel) {
            this.panel.webview.postMessage({
                command: 'setLoading',
                data: state
    ...
    private async handleMessage(message: any) {
        ...
        switch (message.command) {
            ...
            case 'openFile':
                ...
                break;
            case 'restartMCP':
                await this.handleRestartMCP(message as { silent?: boolean });
                break;
            case 'checkMCPHealth':
            ...
            try {
                ...
                if (this.panel) {
                    this.panel.webview.postMessage({
                        command: 'connectResult',
                        success: false,
                        error: (error as Error).message || 'Failed to connect with API key'
                    });
                }
            }
        } else {
    ...
    private async deleteTask(taskId: string, taskTitle: string) {
        try {
            const taskService = getTaskService();
    ...
    private async moveTaskToDone(taskId: string) {
        try {
            const taskService = getTaskService();
    ...
    private async updateTaskAvailability(taskId: string, availabilityStatus: 'available' | 'hold') {
        try {
    ...
    private async reopenTask(taskId: string, reason: string) {
        try {
            const taskService = getTaskService();
    ...
    private async reopenTaskWithComment(taskId: string, reason: string) {
        try {
            const taskService = getTaskService();
            ...
            const action = await vscode.window.showInformationMessage(
                ...
                `Task is now "In Progress". Click "Copy Prompt" to copy a message, then paste it in chat to wake up the AI agent.`,
                { modal: true },
                '📋 Copy Prompt',
                'Cancel'
            );
    ...
    private async handleRequestCommentInput(taskId: string) {
    ...
    private async handleRequestReopenReason(taskId: string) {
    ...
    private async handleRequestStatusChange(taskId: string, currentStatus: string) {
        const statusOptions = [
    ...
    private async handleAddComment(taskId: string, comment: string) {
        try {
            if (!comment || !comment.trim()) {
    ...
    private async handleChangeTaskStatus(taskId: string, newStatus: string) {
        try {
            ...
            vscode.window.showErrorMessage('Failed to change status: ' + (error as Error).message);
        }
    }

    // Remove old mock implementation and keep only these new methods
    private async answerAgentQuestion(data: any) {
        try {
    ...
    private async handleAIQuestionAnswer(questionId: string, answer: string, taskId?: string, qaData?: any) {
        try {
            console.log(`AI Question answered: ${questionId} - ${answer}`);
            
            // Stop notification sound loop immediately
            this.panel?.webview.postMessage({
                command: 'stopNotificationSound'
            });
            
            // If taskId and qaData are provided, save Q&A to task
            if (taskId && qaData) {
                const taskService = getTaskService();
                const localStorage = (taskService as any).localStorage;
                ...
                if (task) {
                    ...
                    const questionIndex = qaHistory.findIndex((qa: any) => 
                        qa.question && qa.question.id === questionId
                    );
                    ...
                    const answeredQuestions = updatedQA.filter((qa: any) => qa.answer !== undefined && qa.answer !== null).length;
                    ...
                    if (remainingQuestions === 0 && totalQuestions > 0) {
                        ...
                        const action = await vscode.window.showInformationMessage(
                            `✅ All questions answered for Task #${taskId}!\n\n` +
                            `Click "Copy Prompt" to copy a message, then paste it in chat to wake up the AI agent.`,
                            { modal: true },
                            '📋 Copy Prompt',
                            'Cancel'
                        );
            ...
            vscode.window.showErrorMessage('Failed to save answer: ' + (error as Error).message);
        }
    }

    private async handleAIQuestionSkip(questionId: string) {
        try {
    ...
    public showAIQuestion(taskId: string, question: any) {
    ...
    private sendQuestionToWebview(taskId: string, question: any) {
        if (!this.panel) {
            return;
        }
    ...
    public updateMCPStatus(status: { isHealthy: boolean; lastCheck: Date; error?: string }) {
        if (!this.panel) {
            return;
        }
    ...
    private async handleRestartMCP(message?: { silent?: boolean }) {
    ...
    private async handleGetCursorProcessTree() {
    ...
    private async getProcessTree(pid: number): Promise<Array<{pid: number, ppid: number, commandLine: string}>> {
        ...
        try {
            ...
            const processes: Array<{pid: number, ppid: number, commandLine: string}> = [];
            ...
            const treeProcesses: Array<{pid: number, ppid: number, commandLine: string}> = [];
            const toCheck = [cursorPid];
            const checked = new Set<number>();
            
            while (toCheck.length > 0) {
                const currentPid = toCheck.pop()!;
                if (checked.has(currentPid)) continue;
    ...
    private async handleCheckMCPHealth() {
        console.log('[Auxly MCP] 🔍 Health check requested from webview');
        
        try {
            // Detect the current editor
            const editor = detectEditor();
            const editorName = getEditorDisplayName(editor);
            
            // For Cursor, check the MCP API
            if (editor === 'cursor') {
                const cursorAPI = (vscode as any).cursor;
                
                if (!cursorAPI || !cursorAPI.mcp) {
                    console.log('[Auxly MCP] ⚠️ Cursor MCP API not available');
                    this.sendMCPHealthStatus(false, `${editorName} MCP API not available`);
                    return;
                }

                // For Cursor: If extension is running, MCP is registered
                console.log('[Auxly MCP] ✅ Extension is active');
                console.log(`[Auxly MCP] ✅ MCP is registered with ${editorName} (extension-auxly)`);
                console.log(`[Auxly MCP] ✅ ${editorName} manages MCP process lifecycle automatically`);
                console.log('[Auxly MCP] ✅ Status: Registered');
                
                this.sendMCPHealthStatus(true);
            } else {
                // For other editors (Windsurf, PearAI, Trae): MCP configured via config file
                console.log(`[Auxly MCP] ✅ Extension is active in ${editorName}`);
                console.log(`[Auxly MCP] ✅ MCP is configured for ${editorName} (via config file)`);
                console.log('[Auxly MCP] ✅ Status: Configured');
    ...
    private sendMCPHealthStatus(isHealthy: boolean, error?: string) {
        if (this.panel) {
            this.panel.webview.postMessage({
...
<head>
    ...
    <style>
        * {
        ...
        .badge-feature { background: #1f6feb; }
        .badge-bugfix { background: #da3633; }
        .badge-refactoring { background: #8957e5; }
        .badge-integration { background: #f778ba; }
        .badge-ui { background: #58a6ff; }
        .badge-research { background: #6e7681; }
        .badge-documentation { background: #57ab5a; }
        .badge-testing { background: #d29922; }
        .badge-planning { background: #768390; }
        .badge-review { background: #9d4edd; }
        .badge-question { background: #f2c94c; color: #000; }
        ...
        window.addEventListener('message', event => {
            ...
            switch (message.command) {
                ...
                case 'stopNotificationSound':
                    // Stop notification sound loop when question is answered
                    console.log('🔇 Stopping notification sound loop');
                    stopNotificationLoop();
                    break;
                ...
                return \`
                    <div class="task-card \${aiWorkingClass} \${holdClass}" data-task-id="\${task.id}">
                        ...
                        <div class="task-meta">
                            ...
                            \${task.category ? \`<span class="badge-category badge-\${task.category}">\${task.category}</span>\` : ''}
                        </div>
                        \${tagsHtml}
                        \${holdStatusBtn}
                        \${changeStatusBtn}
            ...
            descTab.innerHTML = \`
                <div class="modal-section">
                    ...
                    <div class="modal-badges">
                        ...
                        \${task.category ? \`<span class="modal-badge badge-category badge-\${task.category}">\${task.category.toUpperCase()}</span>\` : ''}
