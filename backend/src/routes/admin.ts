...
import { enforceDeviceLimit } from '../services/device-tracking.service';
...
router.delete('/devices/:id', async (req: Request, res: Response) => {
  try {
    ...
    await query(
      'DELETE FROM api_key_devices WHERE id = $1',
      [id]
    );
...
router.post('/create-admin', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Create admin user
    const result = await query(
      `INSERT INTO users (email, password_hash, is_admin, is_blocked) 
 ...
 */
router.post('/users/:id/device-limit', async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    // Get current limit
    const currentLimitResult = await query(
    ...
    await query(
      `INSERT INTO device_limit_audit_log 
       (user_id, admin_id, old_limit, new_limit, reason)
       VALUES ($1, $2, $3, $4, $5)`,
      [id, adminUser.id, oldLimit, maxDevices, reason || null]
    );

    // If limit was lowered, enforce it by disconnecting excess devices
    let disconnectedCount = 0;
    if (maxDevices < oldLimit) {
      const enforceResult = await enforceDeviceLimit(parseInt(id), maxDevices);
 ...
 */
router.get('/users/:id/device-limit-history', async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
 ...
 */
router.post('/users/:id/plan', async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    const subResult = await query('SELECT id, plan_tier FROM subscriptions WHERE user_id = $1', [id]);
...
router.delete('/users/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (hard === 'true') {
      ...
      await query('DELETE FROM device_limit_audit_log WHERE user_id = $1 OR admin_id = $1', [id]);
      
      // 2. Delete usage logs
      await query('DELETE FROM usage_logs WHERE user_id = $1', [id]);
      ...
      // 5. Delete API keys
      await query('DELETE FROM api_keys WHERE user_id = $1', [id]);
