 ...
 */
router.post('/fix-user-3-subscription', async (_req: Request, res: Response) => {
  try {
 ...
 */
router.post('/fix-device-info', async (_req: Request, res: Response): Promise<void> => {
  try {
    // Get current devices
    const devices = await query(
      'SELECT * FROM api_key_devices WHERE api_key_id = $1',
      [15]
    );
 ...
 */
router.post('/check-admin-status', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await query(
      'SELECT id, email, is_admin, is_blocked FROM users WHERE email = $1',
      [email.toLowerCase()]
    );
...
router.post('/set-admin', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await query(
      'UPDATE users SET is_admin = true WHERE email = $1 RETURNING id, email, is_admin, is_blocked',
      [email.toLowerCase()]
    );
