        ...
        try {
          ...
          if (result.rows.length > 0) {
            ...
            result = await pool.query(
              'INSERT INTO users (email, password_hash, oauth_provider, oauth_id, trial_start, trial_end, trial_status) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
              [email, '', 'github', profile.id, trialStart, trialEnd, 'active']
            );
            user = result.rows[0];
      ...
      async (_accessToken: string, _refreshToken: string, profile: any, done: any) => {
        try {
          ...
          if (result.rows.length > 0) {
            ...
            result = await pool.query(
              'INSERT INTO users (email, password_hash, oauth_provider, oauth_id, trial_start, trial_end, trial_status) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
              [email, '', 'google', profile.id, trialStart, trialEnd, 'active']
            );
            user = result.rows[0];
      ...
      async (_accessToken: string, _refreshToken: string, profile: any, done: any) => {
        try {
          ...
          if (result.rows.length > 0) {
            ...
            result = await pool.query(
              'INSERT INTO users (email, password_hash, oauth_provider, oauth_id, trial_start, trial_end, trial_status) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
              [email, '', 'microsoft', profile.id, trialStart, trialEnd, 'active']
            );
            user = result.rows[0];
            console.log('✅ New user created:', user.id);
