...
exports.getTrialInfo = getTrialInfo;
exports.initializeTrial = initializeTrial;
exports.upgradeTrial = upgradeTrial;
...
async function getTrialInfo(userId) {
    try {
        const result = await (0, connection_1.query)(`SELECT trial_start, trial_end, trial_status 
       FROM users 
       WHERE id = $1`, [userId]);
        ...
        if (daysRemaining <= 0 && user.trial_status === 'active') {
            await (0, connection_1.query)(`UPDATE users SET trial_status = 'expired' WHERE id = $1`, [userId]);
 ...
 */
async function initializeTrial(userId) {
    try {
        // Check if trial already exists
        const existing = await (0, connection_1.query)(`SELECT trial_start FROM users WHERE id = $1`, [userId]);
       ...
       SET 
         ...
         trial_status = 'active'
       WHERE id = $1
       RETURNING trial_start, trial_end, trial_status`, [userId]);
 ...
 */
async function upgradeTrial(userId) {
    try {
        await (0, connection_1.query)(`UPDATE users SET trial_status = 'upgraded' WHERE id = $1`, [userId]);
...
async function hasValidAccess(userId) {
    try {
        ...
        const subscription = await (0, connection_1.query)(`SELECT plan_tier, status FROM subscriptions WHERE user_id = $1`, [userId]);
