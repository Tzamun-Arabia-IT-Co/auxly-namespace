...
router.post('/generate', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const result = await (0, api_key_service_1.generateNewApiKey)({
            user_id: req.user.id,
            name: name || undefined,
        });
        if (!result.success) {
...
router.get('/list', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const result = await (0, api_key_service_1.listUserApiKeys)(req.user.id);
        if (!result.success) {
            res.status(400).json({ error: result.error });
            return;
        }
...
router.delete('/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const result = await (0, api_key_service_1.revokeApiKey)(key_id, req.user.id);
        if (!result.success) {
 ...
 */
router.get('/verify', api_key_1.authenticateApiKey, async (req, res) => {
    try {
        if (!req.apiKeyUser) {
            res.status(401).json({ error: 'Unauthorized' });
            return;
...
router.get('/usage', auth_1.authenticate, async (req, res) => {
    ...
    `, [req.user.id]);
        ...
        const planResult = await (0, connection_1.query)('SELECT plan_tier FROM subscriptions WHERE user_id = $1', [req.user.id]);
 ...
 * Protected by JWT authentication
 */
router.get('/devices/all', auth_1.authenticate, async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'Unauthorized' });
            return;
...
router.get('/:id/devices', auth_1.authenticate, async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'Unauthorized' });
            return;
        ...
        const result = await (0, device_tracking_service_1.getDevicesForApiKey)(keyId);
        if (!result.success) {
            res.status(400).json({ error: result.error });
            return;
        }
