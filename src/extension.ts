...
import { registerMCPServerWithCursorAPI, unregisterMCPServer } from './mcp';
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    try {
        ...
        const mcpRegistered = await registerMCPServerWithCursorAPI(context);
        if (mcpRegistered) {
            ...
            const retry = await vscode.window.showWarningMessage(
                '⚠️ Auxly MCP Tools registration failed. This is needed for AI integration.',
                'Retry Now',
            ...
            if (retry === 'Retry Now') {
                ...
                const retryResult = await registerMCPServerWithCursorAPI(context);
                if (retryResult) {
...
function registerCommands(context: vscode.ExtensionContext) {
    ...
    const resetMCPCommand = vscode.commands.registerCommand('auxly.resetMCPConfig', async () => {
        const confirm = await vscode.window.showWarningMessage(
            '⚠️ This will re-register the Auxly MCP server.',
            'Re-register',
            'Cancel'
        );
        ...
        if (confirm === 'Re-register') {
            await unregisterMCPServer();
            const success = await registerMCPServerWithCursorAPI(context);
            if (success) {
